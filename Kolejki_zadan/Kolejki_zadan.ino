...
void setup()
{
  ...
  xTaskCreate(MeasureDistanceTask, "MeasureDistance", 100, NULL, 0, NULL);
  xTaskCreate(MyTask1, "Task1", 100, NULL, 1, NULL);
  xTaskCreate(MyTask2, "Task2", 100, NULL, 1, NULL);
  xTaskCreate(MyTask3, "Task3", 100, NULL, 1, NULL);
  xTaskCreate(MyIdleTask, "IdleTask", 100, NULL, 1, NULL);
}
...
static void MeasureDistanceTask(void* pvParameters)
{
  ...
  { 
    ...
    lcd.print(distance);
    lcd.print(" cm");
    xQueueSend(xQueue, &distance, portMAX_DELAY);
...
static void MyTask1(void* pvParameters)
{
  ...
  while(1)
  { 
    if(xQueueReceive(xQueue, &distance, portMAX_DELAY) == pdPASS) {
      if (distance < 10) {
        digitalWrite(2,HIGH);
        digitalWrite(3,LOW); 
        digitalWrite(4,LOW);
